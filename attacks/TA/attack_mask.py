...
import sys
import torch_dct
from attacks.TA.attack_utils import *
import time
...
def get_x_hat_arbitary(args, start_time, x_o: torch.Tensor, net: torch.nn.Module, original_label, init_x=None,dim_num=5):
    times = []
    if get_label(net(x_o)) != original_label:
        return x_o, 1001, [[0, 0.], [1001, 0.]], times
    if init_x is None:
    ...
    return x_hat, queries, intermediate, times
...
class TA:
    ...
    def attack(self, args, inputs, labels, verbose):
        ...
        for i, [input, label] in enumerate(zip(inputs, labels)):
            if verbose: print('[{}/{}]:'.format(i + 1, len(inputs)), end='')
            global probability
            ...
            x_adv, q, intermediate, times = get_x_hat_arbitary(args,x_o=input[np.newaxis, :, :, :], net=self.net,
                                                        original_label=label.reshape(1, ).to(device),
                                                        init_x=best_advs[i][np.newaxis, :, :, :], dim_num=args.dim_num, start_time=start_time)
            

            x_adv_list[i] = x_adv[0]
            norm = torch.norm(x_adv[0] - input, p=2)
            diff = norm / (args.side_length * np.sqrt(3))
            if diff <= 0.1:
                acc[0] += 1
            if diff <= 0.05:
                acc[1] += 1
            if diff <= 0.01:
                acc[2] += 1
            if verbose: print("Top-1 Acc:{} Top-2 Acc:{} Top-3 Acc:{}".format(acc[0] / (i + 1), acc[1] / (i + 1),
                                                                                     acc[2] / (i + 1)))
            queries.append(q)
            norms.append(norm)
        ...
        return x_adv_list, queries, intermediates, max_length,times
