...
import numpy as np
import torch
from attacks.CGBA.utils import clip_image_values
...
class Proposed_attack():
    def __init__(self, model, src_img, mean, std, lb, ub, 
                 ...
                 verbose_control=False, q_limit=30000, time_budget=None, start_time=None):
        self.model = model
    ...
    def Attack(self):
        ...
        for i in range(self.iteration):
            if i > 0:
                if (q_num >= self.q_limit) or time.time()-self.start_time >= self.time_budget:
                    x_adv = clip_image_values(x_adv, self.lb, self.ub)           
                    return x_adv, n_query, norms, times
            ...
            if i%4==0 or i==self.iteration-1 or q_num>=self.q_limit:
                if self.verbose_control:
